...
const createJwtToken = async (payload: Payload) => {
    return new SignJWT({ session: payload.session, expiresAt: payload.expiresAt.toISOString() })
...
export const verifyJwtToken = async (token: string | undefined = '') => {
    try {
        const { payload } = await jwtVerify<Payload>(token, SECRET, { algorithms: [ALGORITHM] })
        return payload
    } catch {
        return null;
    }
