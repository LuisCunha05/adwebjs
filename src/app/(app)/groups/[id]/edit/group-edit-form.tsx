...
export function GroupEditForm({ group, initialResolvedMembers }: GroupEditFormProps) {
  ...
  const rawMemberList = group.member
    ? Array.isArray(group.member)
      ? group.member
      : [String(group.member)]
    : []
  const displayMembers =
    memberList.length > 0 ? memberList : rawMemberList.map((dn: string) => ({ dn }))
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Alterar nome e gerenciar membros no Active Directory.
          </p>
        </div>
      </div>
      ...
      <Card className="max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Nome de exibição e lista de DNs (um por linha) para edição em massa.
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          {displayMembers.length === 0 ? (
            <p className="text-muted-foreground py-4 text-center text-sm">
              Nenhum membro ou não foi possível carregar.
            </p>
          ) : (
