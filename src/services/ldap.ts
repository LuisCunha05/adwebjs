import { Attribute, Change, Client, EqualityFilter, Filter, OrFilter, ResultCodeError } from "ldapts";
import {
...
export class LdapService implements ILdapService {
      ...
      if (LDAP_GROUP_REQUIRED) {
        const groups = Array.isArray(userEntry.memberOf)
          ...
          : userEntry.memberOf
            ? [userEntry.memberOf]
            : [];
        const isMember = groups.some((g) => g === LDAP_GROUP_REQUIRED || String(g) === LDAP_GROUP_REQUIRED);
  ...
  async updateUser(id: string, changes: Record<string, string | number>): Promise<LdapUserAttributes> {
